...
// Update a user
export const updateUser = async (username: string, updatedUser: User): Promise<User> => {
    try{
        const response = await axios.put<User>(`http://localhost:9000/api/Users/${username}` , updatedUser);
        if(response.status !==200 ){
